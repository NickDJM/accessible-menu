...
class BaseMenu {
  ...
  /**
   * The width of screen (in pixels) that the menu will automatically open/close itself.
   *
   * @protected
   *
   * @type {number}
  ...
  _observer = new ResizeObserver((entries) => {
    requestAnimationFrame(() => {
      for (const entry of entries) {
        ...
        if (
          size.inlineSize <= this._autoCollapse &&
          this.elements.controller.isOpen &&
          !this.elements.controller.isSoftLocked
        ) {
  ...
  _handleClick() {
    ...
    function toggleToggle(menu, toggle, event) {
      ...
      if (toggle.isOpen) {
        ...
        toggle.elements.controlledMenu.focusState = "none";
        toggle.isSoftLocked = true;
      } else {
        toggle.isSoftLocked = false;
      }
    }
      ...
      this.elements.controller.dom.toggle.addEventListener("keyup", (event) => {
        ...
        if (key === "Space" || key === "Enter") {
          ...
          if (this.elements.controller.isOpen) {
            ...
            this.elements.controller.isSoftLocked = true;
          }
        }
      });
    }
  ...
  _handleResize() {
    if (!this.elements.controller || this._autoCollapse <= 0) {
      return;
    }
